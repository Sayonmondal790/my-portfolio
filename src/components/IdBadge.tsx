...
export default function IdBadge() {
  ...
  return (
    <motion.div
      ...
      className="interactive group relative w-64 h-100 rounded-xl border border-neutral-700 bg-neutral-900 shadow-2xl shrink-0 hover:border-cyan-500/50 transition-colors duration-500"
    >
      {/* === THE INTERNAL BACKGROUND LIGHT === */}
      <div className="absolute inset-0 overflow-hidden rounded-xl z-0 pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[150%] h-[150%] bg-cyan-500/20 blur-[70px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
      </div>
      ...
      >
        {/* Profile Photo Area */}
        <div className="w-32 h-32 mb-6 rounded-md overflow-hidden border border-neutral-700 relative shrink-0 group-hover:border-cyan-500/50 group-hover:shadow-[0_0_15px_rgba(34,211,238,0.2)] transition-all duration-500">
           <Image 
             src="/Profile.jpg" 
             alt="Sayon Mondal" 
             fill 
        ...
        {/* Typography Updated with Google Fonts */}
        ...
        <div className="mt-auto w-full pt-4 border-t border-neutral-800 flex justify-between gap-1 opacity-60 group-hover:border-cyan-900/40 transition-colors duration-500">
            {[...Array(20)].map((_, i) => (
              <div 
                key={i} 
                className="bg-neutral-400 group-hover:bg-cyan-400 group-hover:shadow-[0_0_5px_rgba(34,211,238,0.8)] transition-all duration-500 h-8" 
